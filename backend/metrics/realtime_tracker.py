...
class RealtimeMetricsTracker:
    ...
    def __init__(self, window_size: int = 200) -> None:
        ...
        # 신뢰도 보정 (ECE 계산용)
        self._conf_bins: list[list[float]] = [[] for _ in range(10)]  # 10 bins
        self._acc_bins: list[list[int]] = [[] for _ in range(10)]
    ...
    def record(
        ...
        if result.classify_result:
            ...
            if ground_truth is not None:
                pred_0idx = result.classify_result.target_id - 1
        ...
        if result.sequence_result:
            ...
            if not result.sequence_result.accepted:
        ...
        # T1 시작 감지 (WAIT_T2 상태로 전환 = T1 확정)
        if (result.sequence_result and result.sequence_result.accepted
                and result.classify_result
                and result.classify_result.target_id == 1):
            self._seq_started += 1
    ...
    def current_metrics(self) -> MetricsSnapshot:
        ...
        # ── Stage 2: Gate ─────────────────────────────────────────────────
        gate_pass_count = sum(1 for r in results if r.gate_result.is_target)
        ...
        if labeled_pairs:
            ...
            y_pred = [r.classify_result.target_id - 1 for r, _ in labeled_pairs]
            try:
                ...
                cm_raw = confusion_matrix(y_true, y_pred, labels=list(range(max(4, n_classes)))).tolist()
                cm = cm_raw
    ...
    def _compute_ece(self) -> float:
        """Expected Calibration Error (GT 있는 빈만 계산)"""
        total_samples = 0
        ece = 0.0
        for conf_list, acc_list in zip(self._conf_bins, self._acc_bins):
            if not acc_list:
                continue
            n = len(acc_list)
            avg_conf = float(np.mean(conf_list[-n:]))
    ...
    def reset(self) -> None:
        ...
        self._al_queries = 0
        self._al_hits = 0
        self._conf_bins = [[] for _ in range(10)]
        self._acc_bins = [[] for _ in range(10)]
